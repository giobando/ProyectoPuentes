...
def trunk(dato):                # Redondea a 4 decimales!!
...
with open(csvfile_path, 'a') as csvfile:
    if (exists_flag == 0):
        csvfile.write("ID;sensor;Dato;eje1;eje2;tiempo;dato1;dato2;datoTime\n")
    while(START):
        ackPL = [1]
        ...
        string = ""
        for n in receivedMessage:   # print("Traduciendo el mensaje recibido...")
            if ((n >= 32 )and (n <= 126)):                
                string += chr(n)
        print("Recibido: {}".format(string))   
