...
##    indexMaxValue = self.get_PeakFFT(magList, frecuencyList)##    print "Peak at {0}Hz = {1}".format(frecuencyList[indexMaxValue],magList[indexMaxValue])
    plt.clf()      
    plt.plot(frecuencyList[2:], magList[2:], linewidth=0.3)
##      plt.ylim(ymax = 0.002)
    ...
    plt.title('Frequency Domain (' + tituloGrafica + ') con Hamming')
    plt.show()
...
f = fourier()
ss = 1
valuelist =  z[ss*f.cantidadMuestras:(1+ss)*f.cantidadMuestras]
print("listo")
