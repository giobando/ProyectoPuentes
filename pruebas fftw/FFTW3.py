...
class fourier:
  ...
  def plot_to_Fourier(self, vib_data, titulo):
    ...
    # FFT algorithm
    x_input = x_input  = pyfftw.empty_aligned(N, dtype='float32') # para uso del SIMD
    plan = pyfftw.FFTW(x_input,threads=16)
    pyfftw.interfaces.cache.enable() # Turn on the cache for optimum performance
    
    data_fourier = fft(vib_data) 
    ejeY = 2 / self.N * np.abs(data_fourier[0 : np.int(self.N / 2)]) 
